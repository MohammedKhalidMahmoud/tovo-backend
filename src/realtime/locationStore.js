 ...
 *
 * Shape: Map<captainId, { lat, lng, heading, ts }>
...
const set = (captainId, { lat, lng, heading = null }) => {
  store.set(captainId, { lat, lng, heading, ts: Date.now() });
  console.log(store)
};
 ...
 * @returns {{ id: string }[]}
 */
const getNearby = (pickupLat, pickupLng, radiusKm = 10) => {
